...
from MusicGameBot import GG_ID, OO_ID, AU_ROLE_ID, OO_ROLE_ID, OO_JOIN_COMMENT
...
class OtherCommands(commands.Cog):
    ...
    async def fast(self, ctx: Context):
        await ctx.send('arcaea:-／Ravon:-／Deemo:-,-／Cytus:-／DJMAX:-／OverRapid:-／dynamix:up arrow(decrease)／d4dj:音ズレ調整-判定調整+／pjsekai:+／malody:chart+, hit-, graphic-／voez:-／TAKUMI³:+')
    
    @commands.command(aliases=['slow'])
    async def late(self, ctx: Context):
        await ctx.send('arcaea:+／Ravon:+／Deemo:+,+／Cytus:+／DJMAX:+／OverRapid:+／dynamix:down arrow(increase)／d4dj:音ズレ調整+判定調整-／pjsekai:-／malody:chart-, hit+, graphic+／voez:+／TAKUMI³:-')
    ...
    async def mhelp(self, ctx: Context):
        ...
        embed.add_field(name='!quizmode + 難易度', value='クイズの難易度を指定して画像の切り取られる面積を変更します。難易度指定はhardかnormalのみ')
        ...
        embed.add_field(name='!fast/!late', value='それぞれ早グレと遅グレが出るときの判定の動かす方向を出します', inline=False)
        embed.add_field(name='!deemocalibration', value='deemoの曲ごとの判定調整の値を出します', inline=False)
