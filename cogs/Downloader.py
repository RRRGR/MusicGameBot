...
class DLsonglist:
            ...
            if "Song" in _.columns:
                for data in _["Song"]:
                    if pd.isnull(data):
                        ...
                        continue
                    try:
                        arcaeadic[data.replace("エイプリルフール限定楽曲", "")].append(
                            levellist[counter]
                        )
                    except IndexError:
                        break
    ...
    def get_sdvxdic(self):
        ...
        for i in levellist:
            ...
            songlist = [
                _.next_sibling.next_sibling
                for _ in elms
                if _.next_sibling.next_sibling != "\n"
                and _.next_sibling.next_sibling != None
                and "<script" not in str(_.next_sibling.next_sibling)
            ]
            for _ in songlist:
...
class DLimage:
    ...
    def get_diff(self, model):
        ...
        imagelist = [
            _.replace(f"Quiz/{model}image/", "").replace(".jpg", "").replace(".png", "")
            for _ in imagelist
        ]
    ...
    def get_diff_sdvx(self, songdic):
        ...
        imagelist = [
            _.replace("Quiz/sdvximage/", "").replace(".jpg", "").replace(".png", "")
            for _ in imagelist
        ]
        ...
        for k, v in newsongdict.items():
            ...
            for d in difflist:
                try:
                    ...
                    imgurl = (
                        "https://www.sdvx.in"
                        + v[:-3].replace("/js", "/jacket").replace("sort", d)
                        + ".png"
                    )
    ...
    async def dl_arcaeaimage(self, ctx):
        ...
        for _ in new_song:
            ...
            elms = soup.find(
                src=re.compile(
                    "https://cdn.wikiwiki.jp/to/w/arcaea/" + urllib.parse.quote(_)
                )
            )
            try:
        ...
        for _ in new_song:
            ...
            try:
                ...
                try:
                    ...
                    print(url)
                    elms = soup.find(
                        src=re.compile("https://cdn.wikiwiki.jp/to/w/deemo/")
                    )
                    print(elms)
                    imgurl = elms.attrs["src"]
                    imgpath = f"Quiz/deemoimage/{self.convert_half_letter(_)}.jpg"
